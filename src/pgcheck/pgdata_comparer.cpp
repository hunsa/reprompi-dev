...
const double PGDataComparer::critical_t_values[] = {0, 6.314, 2.919986, 2.353363, 2.131847, 2.015048, 1.943180,
...
static double t_test(StatisticValues alternative_values, StatisticValues default_values) {
  ...
  double standard_error = sqrt((alternative_df * alternative_values.variance + default_df * default_values.variance) /
                               (alternative_df + default_df));
  return sqrt((alternative_values.size * default_values.size) / (alternative_values.size + default_values.size)) *
         ((alternative_values.mean - default_values.mean) / standard_error);
}
...
PGCompareResults PGDataComparer::get_results_grouped() {
