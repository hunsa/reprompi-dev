...
static double job_runtime   = 0.0;
static double job_start_time= 0.0;
static int stop_flag = 0;

void roundtimesync_parse_options(int argc, char **argv, reprompi_roundsync_params_t* opts_p) {
    int c;
...
static int roundtimesync_stop_synchronization(void) {
  ...
  if (current_runtime >= job_runtime) {
    // stop job
...
static void roundtimesync_init_module(int argc, char** argv, reprompib_sync_module_t* clock_sync) {
  roundtimesync_parse_options(argc, argv, &parameters);
